...
describe('updatePremise action', () => {
    ...
    const premiseData = { name: 'Test Premise',
    ...
    mockAxios.onPut(`http://localhost:5050/api/premise/${premiseId}`).reply(200, mockResponse);

    const expectedActions = [
        { type: PremiseActionTypes.UPDATE_PREMISE_BEGINS },
        { type: PremiseActionTypes.UPDATE_PREMISE_SUCCESS, payload: mockResponse }
      ];
